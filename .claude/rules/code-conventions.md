...
- All colors are defined in `src/renderer/src/theme.ts` (Catppuccin Mocha tokens). Never hardcode color values inline — always import from `theme.ts`. Use `colorsByTheme[theme].tokenName` in components that receive a `theme` prop; use `colors.tokenName` (dark alias) only where theme context is unavailable. Read `theme.ts` to see available token names before writing any color reference.
